...
#
#
# This is a Python module that allows driving a XXXXX robot.
...
DRIVER_VERSION = "RoboDK Driver for XXX"
...
# ----------- communication class for XXX robots -------------
# This class handles communication between this driver (PC) and the robot
class ComRobot:
    """Robot class for programming XXX robots"""
...
def RunCommand(cmd_line):
    ...
    if cmd_line == "":
        ...
        if ROBOT.SendCmd(xxxxxxxxxxxxxx):
    ...
    elif cmd_line.startswith("SETTOOL"):
        UpdateStatus(ROBOTCOM_WORKING)
        # Set the Tool reference frame provided the 6 XYZWPR cmd_values by RoboDK
        if ROBOT.SendCmd(xxxxxxxxxxxx):
            # Wait for command to be executed
            if ROBOT.recv_acknowledge():
                # Notify that we are done with this command
                UpdateStatus(ROBOTCOM_READY)

    elif cmd_line.startswith("PAUSE"):
        UpdateStatus(ROBOTCOM_WAITING)
        # Run a pause
        if ROBOT.SendCmd(xxxxxxxxxxxxx):
            # Wait for command to be executed
            if ROBOT.recv_acknowledge():
                # Notify that we are done with this command
                UpdateStatus(ROBOTCOM_READY)

    elif cmd_line.startswith("SETROUNDING"):
    ...
    elif cmd_line.startswith("SETDO"):
        UpdateStatus(ROBOTCOM_WORKING)
        # dIO_id = cmd_values[0]
        # dIO_value = cmd_values[1]
        if ROBOT.SendCmd(xxxxxxxxxxx):
            # Wait for command to be executed
            if ROBOT.recv_acknowledge():
                # Notify that we are done with this command
                UpdateStatus(ROBOTCOM_READY)

    elif cmd_line.startswith("WAITDI"):
        UpdateStatus(ROBOTCOM_WORKING)
        # dIO_id = cmd_values[0]
        # dIO_value = cmd_values[1]
        if ROBOT.SendCmd(xxxxxxxxxxxxx):
    ...
    elif cmd_line.startswith("RUNPROG"):
        UpdateStatus(ROBOTCOM_WORKING)

        ROBOT.SendCmd(xxxxxxxxxxx)
    ...
    elif cmd_line.startswith("POPUP "):
        UpdateStatus(ROBOTCOM_WORKING)
...
def RunMain():
    ...
    # Flush version
    print_message("RoboDK Driver v1.0 for robot controllers")
